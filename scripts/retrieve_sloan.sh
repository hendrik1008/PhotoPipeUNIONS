...
python3 @RUNROOT@/@SCRIPTPATH@/SDSS_dataquery.py SDSSDR10 STARS $RAmin $RAmax $DECmin $DECmax \
		    > $md/${FIELD}_sdssdr10_stars.dat 

NLINES=`wc -l $md/${FIELD}_sdssdr10_stars.dat | awk '{print $1}'`
if [ -f $md/${FIELD}_sdssdr10_stars.dat ] && [ ${NLINES} -gt 3 ]
then
    awk '{if (NR>1) print $0,1.033333e-4, 1.033333e-4, 0., 0}' \
	    $md/${FIELD}_sdssdr10_stars.dat | \
	    awk 'BEGIN{i=1}!/#/{print i,$0;i++}' \
	    > $md/${FIELD}_sdssdr10_stars.tmp.dat_$$
    asctoldac -a $md/${FIELD}_sdssdr10_stars.tmp.dat_$$ \
	      -o $md/${FIELD}_sdssdr10_stars.tmp.cat_$$ \
	      -c @RUNROOT@/@CONFIGPATH@/asctoldac_sdssdr10_stars_UNIONS.conf
    ldacaddkey -i $md/${FIELD}_sdssdr10_stars.tmp.cat_$$ \
	       -o $md/${FIELD}_sdssdr10_stars.cat -t FIELDS \
	       -k CRVAL1 0.0 DOUBLE ""\
	       CRVAL2 0.0 DOUBLE ""\
	       CRPIX1 0.0 DOUBLE ""\
	       CRPIX2 0.0 DOUBLE ""\
...
python3 @RUNROOT@/@SCRIPTPATH@/SDSS_dataquery.py SDSSDR10 GALZ $RAmin $RAmax $DECmin $DECmax \
		    > $md/${FIELD}_sdssdr10_galz.dat 

NLINES=`wc -l $md/${FIELD}_sdssdr10_galz.dat | awk '{print $1}'`
if [ -f $md/${FIELD}_sdssdr10_galz.dat ] && [ ${NLINES} -gt 3 ]
then
    awk '{if (NR>1) print $0,1.033333e-4, 1.033333e-4, 0., 0}' \
	    $md/${FIELD}_sdssdr10_galz.dat | \
	    awk 'BEGIN{i=1}!/#/{print i,$0;i++}' \
	    > $md/${FIELD}_sdssdr10_galz.tmp.dat_$$
    asctoldac -a $md/${FIELD}_sdssdr10_galz.tmp.dat_$$ \
	      -o $md/${FIELD}_sdssdr10_galz.tmp.cat_$$ \
	      -c @RUNROOT@/@CONFIGPATH@/asctoldac_sdssdr10_galz_UNIONS.conf
    ldacaddkey -i $md/${FIELD}_sdssdr10_galz.tmp.cat_$$ \
	       -o $md/${FIELD}_sdssdr10_galz.cat -t FIELDS \
	       -k CRVAL1 0.0 DOUBLE ""\
	       CRVAL2 0.0 DOUBLE ""\
	       CRPIX1 0.0 DOUBLE ""\
	       CRPIX2 0.0 DOUBLE ""\
