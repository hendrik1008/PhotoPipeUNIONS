...
#
# Input: PS catalogue and GAaP catalogue
#
...
bash @RUNROOT@/@SCRIPTPATH@/make_make_ssc_conf -i $wd/tmp2.cat_$$ -c 1 | \
    gawk 'BEGIN{FS="="}{if ($1=="COL_NAME") printf "%s_PS\n",$0; else print $0}' \
	>> $wd/make_ssc.conf_$$
...
make_ssc -i ${wd}/tmp3.cat_$$ ${wd}/tmp4.cat_$$ \
             -o ${wd}/merg_PS_comp.cat_$$ \
             -c $wd/make_ssc.conf_$$

ldacrentab -i $wd/merg_PS_comp.cat_$$ -o $wd/${base}_PS.cat \
		-t PSSC OBJECTS

rm $wd/*_$$

python @RUNROOT@/@SCRIPTPATH@/phot_offset_FITS_PS.py \
       $wd/${base}_PS $wd/${base}_PS.cat $band $field \
       > $wd/${base}_PS_${band}_offset.asc
