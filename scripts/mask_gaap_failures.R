...
for (filter in c('Z','Y','J','H','Ks')) { 
  ...
  ind<-mask$imDat[cbind(floor(cat$Xpos),floor(cat$Ypos))]>0
  ind2<-which(ind & cat[[paste0("FLAG_GAAP_",filter)]]==0)
  ind<-which(ind)
  ...
  points(cat$Xpos[ind],cat$Ypos[ind],pch='.',col=ifelse(cat[[paste0("FLAG_GAAP_",filter)]][ind]==0,hsv(0.5,a=0.2),hsv(0.05,a=0.2)))
