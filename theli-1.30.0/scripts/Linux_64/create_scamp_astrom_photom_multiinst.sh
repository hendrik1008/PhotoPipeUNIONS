...
#                      calibration of THELI sets with scamp V2.0.4
#                      (or later)
# -----------------------------------------------------------------------
...
#
# 25.09.2016:
# The catalogue list to be treated by scamp was created from the originally
# present detection catalogues. However, during filtering for good objects,
# several chips could end up with no objects and hence data for these
# chips is no longer present. This led to errors within the script. I fixed
# the creation of input list so that this does no longer lead to errors.
# However, the astrometric solution itself with such data probably cannot
# be regarded useful and such data should not even enter script processing!
#
# 28.03.2018:
# I introduced mode SINGLE which calibrates single images with a distortion
# degree of 1 (fixing CD-matrix only).
...
function printUsage
{
  ...
  echo "    -k LONG/SHORT/SINGLE          (default: LONG)"
  echo ""
  ...
  echo "    data). The distortion degree in this case is 2 (it is 3 for long"
  echo "    exposures). SINGLE should be used to calibrate individual exposures"
  echo "    with no internal overlap to other images (distortion degree is 1)."
  echo " "
...
DATAKIND=LONG # LONG, SHORT esposed data or SINGLE images
PROCESSDIR=""
STARCAT=2MASS
...
do
  ...
  if [ -s ${TEMPDIR}/files.txt_$$ ]; then
    ...
    # we also require a minimum size (semi minor axis)
    SIZEQUAN="B_IMAGE"
    SIZEVAL="1.7"
    ...
    # not be present (bad chips or chips that returned no output objects
    ...
    # Now the list of all available individual catalogues. It is later
    # necessary to check whether certain catalogues are
    # present or not (The file '${TEMPDIR}/files.txt_$$' was created above).
    ${P_GAWK} -F/ '{print $NF}' ${TEMPDIR}/files.txt_$$ | \
      sed -e 's/_[1-9][0-9]*'${ENDING[${DIR}]}'.cat//' | sort | uniq > \
      ${TEMPDIR}/files_basenames.txt_$$

    ${P_FIND} . -maxdepth 1 -mindepth 1 -name \*${ENDING[${DIR}]}.ldac | \
      ${P_GAWK} -F/ '{print $NF}' > ${TEMPDIR}/present.txt_$$

    grep -f ${TEMPDIR}/files_basenames.txt_$$ ${TEMPDIR}/present.txt_$$ > \
      ${TEMPDIR}/images_dir.txt_$$

    cat ${TEMPDIR}/images_dir.txt_$$ >> ${TEMPDIR}/allscampfiles.txt_$$

    EXPOSURES=`${P_GAWK} -F/ '{print $NF}' ${TEMPDIR}/images_dir.txt_$$ | \
               sed -e 's/_[1-9][0-9]*'${ENDING[${DIR}]}'.ldac//' | \
               sort | uniq`
...
if [ -s ${TEMPDIR}/allscampfiles.txt_$$ ]; then
  ...
  if [ "${DATAKIND}" = "SINGLE" ]; then
    DISTORTDEG="-DISTORT_DEGREES 1"
  fi
