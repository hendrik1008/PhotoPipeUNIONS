...
/*
	$Id: fits_p.c,v 1.3 2018/03/28 12:06:21 thomas Exp $
	$Author: thomas $
	$Date: 2018/03/28 12:06:21 $
	$Revision: 1.3 $
*/
...
/* HISTORY COMMENTS (TE)
   ...
   22.09.2014:
   DECam seems to support 'long strings' in their PROGRAM keywords.
   These are strings that do not find in one FITS line but which are
   continued on following lines with a 'CONTINUE' command. For the
   moment we just treat these CONTINUE lines as HISTORY and COMMENT
   lines and we do not care about the string they represent!
*/
...
char * qfits_getkey(char * line)
{
  ...
  /* Sort out special cases: HISTORY, COMMENT, CONTINUE, END do
     not have = in line */
...
char * qfits_getvalue(char * line)
{
  ...
  /*
   * HISTORY/COMMENT/CONTINUE have for value everything else on the
   * line, stripping blanks before and after. Blank
   * HISTORY/COMMENT/CONTINUE is also accepted.
   */
  memset(value, 0, 81);

  if (!strncmp(line, "HISTORY ", 8) ||
      !strncmp(line, "COMMENT ", 8) ||
      !strncmp(line, "CONTINUE ", 9) ||
      !strncmp(line, "        ", 8)) {
    i=7 ;
