...
/*
	$Id: cache.c,v 1.3 2018/03/28 12:06:21 thomas Exp $
	$Author: thomas $
...
/* 20.07.2015:
   functions qfits_query:
   The return type was changed from int
   to long int. DECam MEF-data can be larger than 2GB in float format
   and hence file-pointers need a varaible type that can hold such
   large numbers.
*/
...
/**
  ...
  @return	a long integer offset, or -1 if an error occurred.
 ...
 */
/*----------------------------------------------------------------------------*/
long int	qfits_query(char * filename, int what)
{
	int	 rank ;
	int	 which ;
	long int answer ;
	...
	if (what & QFITS_QUERY_N_EXT) {
		...
		if (which>=0 && which<=qfits_cache[rank].exts) {
                  answer = (long)qfits_cache[rank].ohdr[which] * (long)FITS_BLOCK_SIZE ;
		}
		qdebug(
		...
		if (which>=0 && which<=qfits_cache[rank].exts) {
                  answer = (long)qfits_cache[rank].data[which] * (long)FITS_BLOCK_SIZE ;
		}
		qdebug(
		...
		if (which>=0 && which<=qfits_cache[rank].exts) {
                  answer = (long)qfits_cache[rank].shdr[which] * (long)FITS_BLOCK_SIZE ;
		}
		qdebug(
		...
		if (which>=0 && which<=qfits_cache[rank].exts) {
                  answer = (long)qfits_cache[rank].dsiz[which] * (long)FITS_BLOCK_SIZE ;
		}
		qdebug(
