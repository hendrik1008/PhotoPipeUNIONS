 ...
 * keywords is only executed if images are resampled. That gave
 * incorrect results if only an image header should be output from
 * swarp - corrected.
 *
 * 19.08.2014:
 * routine init_field:
 * The field size of multi-chip cameras crossing the Ra=0 line could be
 * be determined incorrectly leading to images that were cutted within chips
 * crossing the pole - fixed.
 */
...
/*------------ Test for the lower limit */
              if ((n==lng && fcmp_0_p360(wcsmin1, wcsmin2)) ||
                  (n!=lng && wcsmin1>wcsmin2))
                countmin++;
              if ((n==lng && fcmp_0_p360(wcsmax2, wcsmin1)) ||
                  (n!=lng && wcsmax2>wcsmin1))
                countmin++;
/*------------ Test for the upper limit */
              if ((n==lng && fcmp_0_p360(wcsmax2, wcsmax1)) ||
                  (n!=lng && wcsmax2>wcsmax1))
                countmax++;
              if ((n==lng && fcmp_0_p360(wcsmax1, wcsmin2)) ||
                  (n!=lng && wcsmax1>wcsmin2))
                countmax++;
              }
...
/*---------- Update the "most intersecting" limits */
        ...
        case CENTER_ALL:
          ...
          for (i=1; i<ninput; i++)
            ...
            wcsmax2 = infield[i]->wcs->wcsmax[axis[i*naxis+n]];
/*---------- Test for the lower limit */
            if ((n==lng && fcmp_0_p360(wcsmin[n],wcsmin2)) ||
                (n!=lng && wcsmin[n]>wcsmin2))
              wcsmin[n] = wcsmin2;
/*---------- Test for the upper limit */
            if ((n==lng && fcmp_0_p360(wcsmax2,wcsmax[n])) ||
                (n!=lng && wcsmax2>wcsmax[n]))
              wcsmax[n] = wcsmax2;
            }
          wcs->crval[n] = (wcsmin[n]+wcsmax[n])/2.0;
          if (n==lng)
