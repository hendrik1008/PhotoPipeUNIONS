 ...
 *
 * 14.12.2015
 * The original code only allowed polygon files with the polygon identified
 * 'polygon': I changed that so that also 'POLYGON' is possible. All THELI
 * masks use that identifier.
 */
...
/*-- Examine current input line (discard empty and comment lines) */
    ...
    if (str2 && !(strstr(str2, "polygon")) && !(strstr(str2, "POLYGON")))
      continue;
