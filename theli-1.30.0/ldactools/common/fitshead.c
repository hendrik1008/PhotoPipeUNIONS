 ...
 *
 * 03.09.2018:
 * functions readbintabparam_head and update_head:
 * key comments are now also stored in TCOMM header keywords. Before,
 * they were only 'comments' of the TTYPE keyword.
 * NOTE: Both ways for handling comments of keys are not standardized and
 *       astropy even deletes those comments when recreating FITS tables!
 *
 */
...
/*--map binary-table fields*/
    ...
    sprintf(strk, "TCOMM%-3d", i+1);
    /* if the comment was already read before (comment of TTYPE) and
       if the TCOMM keyword does not exist, the already initialised
       key->comment is not altered by the following. The fitsread
       routine immediately stops if the TCOMM key does not exist.
    */
    fitsread(tab->headbuf, strk, key->comment, H_STRING, T_STRING);
...
/*--Check that the new parameters will fit in the allocated memory space*/
    ...
    if (*key->comment)
      ...
      sprintf(strk, "TCOMM%-3d", i+1);
      fitsadd(tab->headbuf, strk, "");
      fitswrite(tab->headbuf, strk, key->comment, H_STRING, T_STRING);
      }
    if (*key->printf)
      {
      sprintf(strk, "TDISP%-3d", i+1);
